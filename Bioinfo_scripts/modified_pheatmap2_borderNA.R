...
# questa funzione modifica la funzione pheatmap per colorare i bordi della matrice di annotazione diversamente dalla matrice dei dati
...
#=================== MODIFICA funzione heatmap_motor =========================

# la funzione heatmap_motor è la funzione principale utilizzata da pheatmap che genera in concreto il plot  

heatmap_motor2 <- function (matrix, border_color, 
                            border_color_anno = NA , # INSERITO questo nuovo parametro!
                            cellwidth, cellheight, tree_col, 
                            ...
                            hjust_col, vjust_col, angle_col, fmat, fontsize_number, 
                            number_color, gaps_col, gaps_row, labels_row, labels_col, 
                            ...) 
{
  lo = pheatmap:::lo(coln = labels_col, rown = labels_row, nrow = nrow(matrix), 
                     ...
                     fontsize_col = fontsize_col, angle_col = angle_col, 
  ...
  if (!pheatmap:::is.na2(annotation_col)) {
    converted_annotation = pheatmap:::convert_annotations(annotation_col, 
                                                          annotation_colors)
    elem = pheatmap:::draw_annotations(converted_annotation, border_color_anno, 
...
#=================== MODIFICA funzione pheatmap =========================


pheatmap2 <- function(mat, color = colorRampPalette(rev( RColorBrewer::brewer.pal(n = 7, 
                                                                                  name = "RdYlBu")))(100), kmeans_k = NA, breaks = NA, border_color = "grey60", border_color_anno =NA,
                      ...
                      silent = FALSE, na_col = "#DDDDDD", ...)
{
  ...
  if (scale != "none") {
    ...
    if (pheatmap:::is.na2(breaks)) {
      breaks = pheatmap:::generate_breaks(mat, length(color), center = T)
    }
  }
  if (!is.na(kmeans_k)) {
  ...
  if ((class(cluster_rows) == "hclust") || cluster_rows) {
    if (class(cluster_rows) == "hclust") {
      ...
      tree_row = pheatmap:::cluster_mat(mat, distance = clustering_distance_rows, 
  ...
  if ((class(cluster_cols) == "hclust") || cluster_cols) {
    if (class(cluster_cols) == "hclust") {
      ...
      tree_col = pheatmap:::cluster_mat(t(mat), distance = clustering_distance_cols, 
                                        method = clustering_method)
  ...
  if (!pheatmap:::is.na2(legend_breaks) & !pheatmap:::is.na2(legend_labels)) {
    if (length(legend_breaks) != length(legend_labels)) {
      stop("Lengths of legend_breaks and legend_labels must be the same")
    }
  }
  if (pheatmap:::is.na2(breaks)) {
    breaks = pheatmap:::generate_breaks(as.vector(mat), length(color))
  }
  ...
  mat = pheatmap:::scale_colours(mat, col = color, breaks = breaks, na_col = na_col)
  if (pheatmap:::is.na2(annotation_col) & !pheatmap:::is.na2(annotation)) {
  ...
  if (length(annotation) != 0) {
    annotation_colors = pheatmap:::generate_annotation_colours(annotation, 
...
data[1:10, seq(1, 10, 2)] <- data[1:10, seq(1, 10, 2)] + 3
data[11:20, seq(2, 10, 2)] <- data[11:20, seq(2, 10, 2)] + 2
data <- data + rnorm(200, sd = 0.4)
colnames(data) <- paste("P", 1:10, sep = "")
rownames(data) <- paste("G", 1:20, sep = "")

# Crea annotazioni di colonne
annotation_col = data.frame(
  Group = factor(rep(c("Control", "Treatment"), each = 5)),
  Time = 1:10
)
rownames(annotation_col) <- paste("P", 1:10, sep = "")

# funzione standard
pt0 <- pheatmap(data,annotation_col = annotation_col)


# funzione modificata, border_color_anno = NA
pt <- pheatmap2(data, annotation_col = annotation_col, 
                border_color = "black",
                border_color_anno = NA)
